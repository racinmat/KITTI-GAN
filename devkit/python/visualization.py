...
# /opt/project/devkit/matlab/visualization.m

    
@function
def visualization(mode=None,image_dir=None,varargin=None,*args,**kwargs):
    varargin = visualization.varargin
    nargin = visualization.nargin

    if 'init' == mode:
        # create figure using size of first image in repository
        fig=figure(1)
# /opt/project/devkit/matlab/visualization.m:9
        img=imread(sprintf('%s/%010d.png',image_dir,0))
# /opt/project/devkit/matlab/visualization.m:10
        set(fig,'position',cat(100,100,dot(0.8,size(img,2)),dot(dot(0.8,2),size(img,1))))
        h(1).axes = copy(axes('position',cat(0,0.5,1,0.5)))
# /opt/project/devkit/matlab/visualization.m:12
        h[2].axes = copy(axes('position',cat(0,0,1,0.5)))
# /opt/project/devkit/matlab/visualization.m:13
        varargout[1]=h
# /opt/project/devkit/matlab/visualization.m:14
    else:
        if 'update' == mode:
...
# /opt/project/devkit/matlab/visualization.m:25
            ...
            text(0,0,'Not occluded','parent',h[1].axes,'color','g','HorizontalAlignment','left','VerticalAlignment','top','FontSize',14,'FontWeight','bold','BackgroundColor','black')
